...
public class GameMenuRoom(GameManager game) : IView
{
    ...
    public GameMenuRoom(GameManager game, List<Markup> actionLog): this(game)
    {
        ActionLog = actionLog;
    }
    ...
    public void Render()
    {
        ...
        Layout["LLTop"].Update(
            new Panel(
                Align.Center(new Markup($"Pretend this guy\n{Game.Data?.Pet.Icon ?? "#"}\nis in a {Game.Data?.Room.RoomName ?? "Room"}\n[gray](I didnt have time to add ascii art xd)[/]"), VerticalAlignment.Middle)
            )
            {
                Header = new PanelHeader(Game.Data?.Room.RoomName ?? "Room")
            }.Expand()
        );
        ...
        Layout["LLBottom"].Update(
            new Panel(
                Align.Center(new Columns(
                    ...
                    new Panel(new Markup("🏡")){ Header = new PanelHeader("2"), BorderStyle = new Style(Color.Yellow) },
            ...
            )
            {
                ...
                BorderStyle = new Style(Color.White),
                Width = 20,
                Header = new PanelHeader("(num keys to navigate)")
            }.Expand()
        );
