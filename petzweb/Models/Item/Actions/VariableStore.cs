...
public class VariableStore
{
    ...
    public string? GetVariable(string key, string? fallback = null)
    {
        ...
        return _readOnlyVariables.TryGetValue(key, out var readOnlyValue) ? readOnlyValue : fallback;
    }
    ...
    {
        if (!_variables.TryGetValue(key, out var current)) return;
        if (int.TryParse(current, out var currentInt))
        {
            _variables[key] = (currentInt + value).ToString();
        }
        else if (double.TryParse(current, out var currentDouble))
        {
